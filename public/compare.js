...
async function loadEntries(userEmail) {
  let entries = [];
  try {
    // Get the latest entries for the specified user from the service
    const response = await fetch(`/api/entries?email=${encodeURIComponent(userEmail)}`);
    entries = await response.json();

    // Save the entries in case we go offline in the future
    localStorage.setItem('previousEntries', JSON.stringify(entries));
  } catch {
    // If there was an error then just use the last saved entries
    const entriesText = localStorage.getItem('previousEntries');
    if (entriesText) {
